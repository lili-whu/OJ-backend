...
public class RecordSubmitController {
    ...
    @GetMapping("/{id}")
    @UserRoleAnnotation(UserRole.DEFAULT_ROLE)
    public Result<RecordSubmitVO> getRecordSubmit(@PathVariable Long id, HttpServletRequest request) {
        RecordSubmitVO recordSubmitVO = recordSubmitService.getSingleRecordSubmit(id, request);
        return Result.success(recordSubmitVO);
    }
